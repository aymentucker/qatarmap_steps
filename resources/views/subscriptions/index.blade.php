<x-app-layout :assets="$assets ?? []" title='الاشتراكات' :isBanner="true">
    <div class="col-lg-12">
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center">
                <h5 class="mb-0">الاشتراكات</h5>
                <a href="{{ route('subscriptions.create') }}" class="btn btn-primary">إضافة اشتراك</a>
            </div>
            <div class="card-body">
                <div class="fancy-table table-responsive border rounded">
                    <table class="table table-striped mb-0">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td class="text-dark">{{ $subscription->name }}</td>
                                <td class="text-dark">{{ $subscription->type }}</td>
                                <td class="text-dark">{{ $subscription->price }}</td>
                                <td>
                                    <div class="d-flex justify-content-evenly">
                                        <a class="btn btn-primary btn-icon btn-sm rounded-pill ms-2"
                                            href="{{ route('subscriptions.edit', $subscription->id) }}" role="button">
                                            <!-- SVG for edit icon -->
                                        ...
                                        <a class="btn btn-danger btn-icon btn-sm rounded-pill ms-2" href="#" onclick="event.preventDefault(); if(confirm('Are you sure?')) document.getElementById('delete-subscription-{{ $subscription->id }}').submit();" role="button">
                                            <!-- SVG for delete icon -->
                                            <span class="btn-inner">
                                                <svg class="icon-32" width="32" viewBox="0 0 24 24"
                                                    fill="none" xmlns="http://www.w3.org/2000/svg">

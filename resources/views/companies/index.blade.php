<x-app-layout :assets="$assets ?? []" title=' عرض الشركات' :isBanner="true">
    <div class="row">
        <div class="col-lg-12">
            <div class="card">
                <div class="card-body">
                            ...
                            <tbody>
                                @foreach ($companies as $company)
                                    <tr>
                                        ...
                                        <td class="text-dark">{{ $company->manager_phone }}</td>
                                        <td class="text-dark">{{ $company->address }}</td>
                                        <td class="text-dark">{{ $company->users->count() }}</td>
                                        <td>
                                            @if($company->formatted_status == 'Active')
                                                <span class="badge bg-success p-2 text-white">
                                                    نشط <!-- Active -->
                                                </span>
                                            @elseif($company->formatted_status == 'Pending')
                                        ...
                                        <td>
                                            <div class="d-flex justify-content-evenly">
                                                <!-- Example Edit Button -->
                                                <a class="btn btn-primary btn-icon btn-sm rounded-pill ms-2" href="{{ route('companies.edit', $company->id) }}" role="button">
                                                ...
                                                <!-- You might want to use a form here to submit a DELETE request -->
                                                <a class="btn btn-danger btn-icon btn-sm rounded-pill ms-2" href="#" onclick="event.preventDefault(); document.getElementById('delete-form-{{ $company->id }}').submit();" role="button">
                                                    <!-- Delete form -->

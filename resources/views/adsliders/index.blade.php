<x-app-layout :assets="$assets ?? []" title=' البنرات الاعلانية ' :isBanner="true">
    <div class="col-lg-12">
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center">
                <h5 class="mb-0">البنرات الاعلانية</h5>
                <a href="{{ route('adsliders.create') }}" class="btn btn-primary">إضافة بنر اعلاني</a>
            </div>
            <div class="card-body">
                    ...
                    <table class="table table-striped mb-0">
                        <thead>
                                <tr>
                                    ...
                                    <th scope="col">الصورةاو البنر</th>
                            ...
                            <tbody>
                                @forelse ($adSliders as $adSlider)
                                <tr>
                                    <td class="text-dark">{{ $adSlider->name }}</td>
                                    <td>
                                        <img src="{{ $adSlider->image }}" alt="Slider Image" style="width: 100px; height: auto;">
                                    </td>
                                    <td class="text-dark">{{ $adSlider->url_link }}</td>
                                    <td class="text-dark">{{ $adSlider->subscription_period }}</td>
                                    <td> <span class="badge bg-soft-primary p-2 text-primary">{{ $adSlider->end_date->format('Y-m-d') }}</span></td>
                                    <td>
                                        <div class="d-flex justify-content-evenly">
                                            <a class="btn btn-primary btn-icon btn-sm rounded-pill ms-2"
                                            href="{{ route('adsliders.edit', $adSlider->id) }}" role="button">
                                            ...
                                            <a class="btn btn-danger btn-icon btn-sm rounded-pill ms-2" href="#" onclick="event.preventDefault(); document.getElementById('delete-adSlider-{{ $adSlider->id }}').submit();" role="button">
                                                <span class="btn-inner">
                                                    <svg class="icon-32" width="32" viewBox="0 0 24 24"
                                                        fill="none" xmlns="http://www.w3.org/2000/svg">
                                                        <path opacity="0.4"
                                            ...
                                            <form id="delete-adSlider-{{ $adSlider->id }}" action="{{ route('adsliders.destroy', $adSlider->id) }}" method="POST" style="display: none;">
                                                @csrf
                                                @method('DELETE')
                                            </form>
                                        </div>
                                ...
                                @empty
                                <tr>
                                    <td colspan="5" class="text-center">No ad sliders found</td>
                                </tr>
                            @endforelse
                            </tbody>
                        </table>
